...
var removeStopWords = (() => {
  ...
  function removeStopWordsInner(input) {
    const chain = input.trim().split(/\s+/g);
    const result = chain.filter((x) => !stopWords.has(x.toLowerCase().replace(/[^\w\d']/gi, "")));
    return result.join(" ");
  }
  return removeStopWordsInner;
})();
var nlp_default = {
  removeStopWords
};
