...
const stopWords = (() => {
  const STOP_WORDS: {
    [key in StopWordLanguages]: Set<string>;
  } = {
  ...
  function isStopWord(word: string, lang: StopWordLanguages = "english"): boolean {
    return STOP_WORDS[lang].has(word.replace(/[^\w\d']/gi, "").toLowerCase());
  }

  function removeStopWords(input: string, lang: StopWordLanguages = "english"): string {
    const tokens = input.trim().split(/\s+/g);
    const result = tokens.filter((word: string) => !isStopWord(word, lang));
    return result.join(" ");
  }
